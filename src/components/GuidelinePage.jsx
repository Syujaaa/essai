...
export default function GuidelinePage({ mode, onBack }) {
  ...
  // Tambahan Ref untuk melacak index yang sedang dibaca (mencegah bentrok jika user klik manual)
  ...
  // PANDUAN: Hapus instruksi suara 'lanjut' karena sekarang auto-play
  const getVoicePrompt = (index) => {
    if (index < guidelines.length - 1) {
      return ""; // Langsung ke berikutnya, tidak perlu suruh bilang lanjut
    } else {
      return " Ini adalah panduan terakhir. Ucapkan kembali untuk kembali ke menu.";
    }
  };

  // FUNGSI BARU: Membaca secara berurutan (Auto-play sequence)
  const readSection = (index) => {
    currentReadingIndexRef.current = index;
    const section = guidelines[index];
    const text = `${section.title}. ${section.content}${getVoicePrompt(index)}`;

    speakUI(text, () => {
      // Jika ref berubah (artinya user menekan tombol manual saat sedang baca), batalkan auto-next
      if (currentReadingIndexRef.current !== index) return;

      if (index < guidelines.length - 1) {
        const nextIndex = index + 1;
        setActiveSection(nextIndex);
        // Lanjut baca section berikutnya secara otomatis
        readSection(nextIndex);
      } else {
        // Jika sudah di akhir, nyalakan mikrofon untuk menunggu perintah "Kembali"
        startListening();
      }
    });
  };
  ...
  const handleBack = () => {
    currentReadingIndexRef.current = -1; // Hentikan siklus bacaan
    isIntentionalStopRef.current = true;
    window.speechSynthesis.cancel();
    if (recognitionRef.current) {
      try {
  ...
  useEffect(() => {
    if (useVoiceControl) {
      // Langsung panggil fungsi auto-play dari index 0
      readSection(0);
    }

    return () => {
      currentReadingIndexRef.current = -1; // Matikan siklus saat komponen unmount
      isIntentionalStopRef.current = true;
      window.speechSynthesis.cancel();
      if (recognitionRef.current) {
        try {
